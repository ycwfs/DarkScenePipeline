...
（`other` 默认在 `clip_skip_labels` 里）。**默认 `0.5`**；填 `0` 关闭，按模型原始判定输出。
...
`reco_min_conf` 显示的是**实际生效**的值。不填时它应当是 `0.5`（本算子的默认值）；如果那里是
`0.0` 而你并没有填 `0`，说明平台上注册的还是旧版 `suanzi.json`（旧默认是 `0.8`，更早的默认是
`0.0`）——**换了 zip 之后需要在平台上重新导入算子**，否则框架按旧 manifest 下发命令行，新默认
不会生效。

**怎么定这个值**：默认 `0.5` 偏向不漏报——宁可多几条误报，也不让真实动作因为分数差一点而被
压成 `other`。想自己定就先按 `0` 跑一段，看 `events.jsonl` 里 `confidence` 的实际分布，再挑一个
能把误报切掉、又不误伤真实动作的位置。暗光场景下模型对不确定的画面仍会给出某个具名行为，只是分数
偏低，这个阈值就是用来压住那部分的；填得过高会连真实动作一起滤掉——上表 0.9 就是故意设得过高的
示例。
...
| `reco_min_conf` | 行为判定阈值 | Float | `0.5` | 低于该置信度报为「其他」，不显示也不存片段；`0`=关闭 |
