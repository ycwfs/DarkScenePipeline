...
> 取回来之后它是磁盘上一个有限长的文件 → 离线模式；否则实时模式。`rtmp_push_url` 不参与判定：
> 文件输入下该参数被忽略，文件一律**只处理一遍就退出，不循环播放**。

| `video_path` | 模式 | 产出 | 结束方式 |
| --- | --- | --- | --- |
| `rtsp://…`、GB28181、flv/hls 流 | 实时 | 三条 HTTP 流 + 按动作切的片段 mp4 | 常驻，等容器停止或 `run_seconds` 到期 |
| 本地文件 / `hdfs://…` / WebHDFS `http://…mp4` | **离线** | **整段处理后的视频** + 按动作切分的片段视频 + 事件 JSON | **只处理一遍，跑完自行退出** |

日志里会明写这次选了哪种以及为什么，不用猜：

```
[mode] 离线推理：输入是有限长文件（86 MB / 3241 帧 / 25.00 fps），整段只处理一遍、不循环播放，产出整段处理后的视频并按动作切片（命名与实时模式不同），完成即退出
[mode] 实时服务：输入是实时流，常驻运行并按动作切片
```

离线模式的几点差异，都是有意为之：

- **输出既有整段视频，也有按动作切分的片段**。整段视频命名为 `<视频名>-处理后.mp4`，帧率
  与时长同源视频一致（用的是源视频自己的帧率，不是实时模式那个受拉流速度约束的值）；片段
  命名为 `<起始秒>-<结束秒>s-<中文动作>.mp4`（如 `2-3s-喝水.mp4`），按片段在原视频里的
  时间顺序 + 动作类别命名——离线是回放一段已经录好的视频，「什么时候发生的」指的是原视频
  里第几秒，不是墙钟时间，所以不用实时模式那种 `<时间戳>_<动作key>_<序号>.mp4`；片段平铺在
  会话目录下，不再像实时模式那样按行为分子目录（文件名本身已经带了动作类别）。`clip_pre_sec`
  /`clip_post_sec`/`clip_max_sec`/`clip_skip_labels`/`clip_denoise` 同样生效；只有
  `run_seconds` 不生效（整段视频跑完即退出，没有"运行时长"这回事）。
- 识别照常做，事件写进 `events.json` 并在 `session_json` 里给出各行为的计数；`other`
  （默认在 `clip_skip_labels` 里）与被过滤掉的行为不会切出片段，但仍计入 `events.json`。
- 产出（整段视频 + 事件 JSON + 全部片段）同时落 `clip_dir/<会话>/` 与
  `hdfs_output_dir/<会话>/`。**上传失败是致命的**（非零码退出）——批处理跑完就退，产出取不
  回来等于这次白跑；实时模式那边是「上传失败只告警」，因为本地片段还在盘上，这是两种不同的
  取舍。
- 长视频跑的过程中每约 5 秒打一行进度，别让平台的日志面板一片空白：
  ```
  [progress] 1280/3241 帧 (39.5%)  22.1 fps  已用 58s  剩余约 1.5 分钟
  ```
- **切片依赖单卡上一条连续的时间线，与多卡分片（`gpu_ids` 填两张卡）互斥**：分片是把视频切成
  互不重叠的帧段各自独立处理，识别窗口在每段开头重置，也没有任何跨段协调，拼不出一条连续
  时间线来切片。`clip_dir` 默认非空（默认就要产出片段），`gpu_ids` 现在也默认是两张卡
  `"0,1"`，两者冲突时以切片为准——离线模式会自动降级为单卡并打日志说明，不会报错、也不会
  静默丢片段。只有显式把 `clip_dir` 清空，才会真正吃满两张卡跑分片。
...
一个独立的 mp4 存下来。以下命名/目录结构、`clip_pre_sec`/`clip_post_sec`/`clip_max_sec` 的
判定逻辑、去噪、H.264 编码等都是两种模式共用的同一套 `ClipRecorder`；**离线模式的命名和目录
布局不同**（平铺、按时间范围+中文动作命名，见前面「离线模式的几点差异」），下面这套
`<行为>/<时间>_<行为>_<序号>.mp4` 是实时模式的：
...
例如 `/opt/darkpipe/clips/20260101_090000_1_darkpipe_input_024/falling/20260101_091233_falling_0007.mp4`。
同名 `.json` 记录标签、置信度、片段内各标签计数、帧数、帧率、时长与收尾原因。

**`<会话>` 里带输入的标识**，这样落地目录里一排目录能直接看出哪个是哪路输入的结果，不必点开
文件：

| 输入 | 会话名 | 标识取自 |
| --- | --- | --- |
| `rtmp://10.46.79.133/live/darkpipe_input_024` | `20260101_090000_1_darkpipe_input_024`（**后缀**） | 流地址末段，即流名 |
| `/mnt/nfs/videos/demo.mp4` | `demo_20260101_090000_1`（**前缀**） | 文件名主干 |
| `http://…:9870/behavor/演示视频2026-08-19.mp4` | `演示视频2026-08-19_20260101_090000_1` | **原始地址**的文件名 |
| 地址里取不出名字（如 `rtsp://ip:554/`） | `20260101_090000_1` | 退回主机名，再取不到就只剩时间戳 |

位置不同是有意的：离线通常是同一个目录里跑很多个不同的文件，前缀让同一个视频的多次运行排在
一起；实时流是一个流跑很久，按时间排更好找。标识取自**你填的 `video_path` 原文**而不是本地
副本——HDFS 输入下载到容器里一律叫 `input.mp4`，用它命名的话每次都是 `input_…`。
用户名密码（`rtsp://admin:pass@…`）不会进目录名；空格、冒号、斜杠会换成下划线；中文保留。
...
1. **片段包含触发之前的画面。** 识别事件是在动作发生**之后**才产生的，从触发点开始录只能录到
   动作的尾巴，因此有 `clip_pre_sec`（默认 1 秒）的前置缓冲。
...
**写盘与上传都在独立线程上做**，识别主循环只往一个队列里丢帧。**实时模式**下这个队列是
有界的——这是为了守住「实时时延 ≤ 1 秒」这条硬指标，磁盘或 HDFS 抖一下不能把识别拖慢；
队列满时会丢帧并打印累计丢帧数（片段仍会保存，只是略有跳帧），不会阻塞识别。**离线模式没有
「实时」这回事**：输入是已经落地的有限长文件，没有直播时延要保，只要求最终结果完整，所以
这个队列在离线模式下不设上限——写盘线程（尤其 `clip_denoise quality`）跟不上处理速度时，只
会拉长整个任务的耗时，绝不会丢帧或放弃某个片段（`clips_abandoned`/`frames_dropped` 在离线
模式下恒为 0）。
...
- `gpu_ids` 填的是**容器内**卡号，从 0 起连续编号，与宿主机卡号无关。**默认 `"0,1"`，即用满两张卡**；
  只想用一张卡就把本项改成 `0`。
  单卡跑不出上表的 23.8 fps（840 单卡是 12.9 fps），**只分到 1 张卡时 `proc_max_side` 要一并改成 720**——
  平台实际只分到 1 张卡时会自动降级并在日志里提示这一条，不需要每次手动确认。
...
（`other` 默认在 `clip_skip_labels` 里）。**默认 `0.65`**；填 `0` 关闭，按模型原始判定输出。
...
`reco_min_conf` 显示的是**实际生效**的值。不填时它应当是 `0.65`（本算子的默认值）；如果那里是
别的值而你并没有填，说明平台上注册的还是旧版 `suanzi.json`（历史默认依次为 `0.5`、`0.8`、
`0.0`）——**换了 zip 之后需要在平台上重新导入算子**，否则框架按旧 manifest 下发命令行，新默认
不会生效。

**怎么定这个值**：默认 `0.65` 与真实场景微调后的模型配套——微调让模型对真实动作的置信度整体
抬高，阈值相应从早前的 `0.5` 上调，多压掉一段低置信度误报而不再以漏报为代价。想自己定就先按
`0` 跑一段，看 `events.jsonl` 里 `confidence` 的实际分布，再挑一个能把误报切掉、又不误伤真实
动作的位置。暗光场景下模型对不确定的画面仍会给出某个具名行为，只是分数偏低，这个阈值就是用来
压住那部分的；填得过高会连真实动作一起滤掉——上表 0.9 就是故意设得过高的示例。
...
> 行为的压力素材。
>
> **这一整段都是实时模式（1 秒时延指标）才有的取舍。** 离线模式没有时延指标要守，写盘线程
> 跟不上 `quality`/`quality_high` 时只会让整段任务多跑一会儿，不会丢帧、不会弃段——离线场景
> 想要更干净的片段画质，直接用 `quality`/`quality_high` 即可，不需要因为这个原因退回 `fast`。
...
| `reco_min_conf` | 行为判定阈值 | Float | `0.65` | 低于该置信度报为「其他」，不显示也不存片段；`0`=关闭 |
| `label_bar` | 叠加标签条 | Bool | `true` | 演示画面与片段底部是否叠加识别结果 |
| `gpu_ids` | GPU卡号 | String | `0,1` | 逗号分隔，多张时按帧轮询分发；容器内卡号从 0 起编，见下。**默认双卡**，`proc_max_side=840` 已按双卡校准；**只分到 1 张卡时须把 `proc_max_side` 改 720** |
...
| `rtmp_push_url` | 推流地址(可留空) | String | 无（**可留空**） | 推到外部流媒体服务器，支持 `rtmp://ip:1935/应用/流名` 与 `rtsp://ip:8554/流名`（RTSP 固定走 TCP）；留空不推；仅实时流输入生效，文件输入时忽略 |
| `stream_bitrate` | 出流码率上限 | String | `0` | 所有 H.264 出流的码率上限；**默认不限制**（按画面复杂度自适应，画质最好）；链路带宽受限时可填 `4M`/`8M` 等设上限 |
| `max_flv_clients` | FLV并发观看上限 | Int | `4` | 超出返回 503；需要更多观看端请改用推流 |
| `jpeg_quality` | 演示流画质 | Int | `100` | 1-100，调低省带宽；`100` 的 MJPEG 字节量约为 `85` 的 2 倍，带宽或 ffmpeg 吃紧时优先调这个 |
| `max_stream_fps` | 演示流最大帧率 | Float | `15.0` | 出流帧率；**画面右下角显示的就是这个值** |
| `clip_dir` | 片段保存目录 | String | `/opt/darkpipe/clips` | 容器内路径，建议挂 NFS 后本地浏览；离线模式下整段视频、事件 JSON 与切分好的片段也都落这里 |
| `clip_pre_sec` | 片段前置时长(秒) | Float | `1.0` | 向前多保留的秒数 |
| `clip_post_sec` | 片段后置时长(秒) | Float | `1.0` | 最后一次命中**本片段行为**后再录多久才收尾；识别大约每 0.7~1.3 秒一次(取决于卡数)，默认值在双卡下有余量，单卡兜底时余量很薄 |
...
`clip_videos` 给出每段切片在本地与 HDFS 的落地地址，`clips` 给出片段统计
（`clips_saved`/`clips_abandoned` 等），`events_total` / `label_counts` 给出识别事件总数与
各行为计数，`stats` 给出帧数、耗时与帧率。
...
不需要 `-p 8000:8000`（不起服务），也不需要 `--run_seconds`（跑完即退）。产出除了整段
`-处理后.mp4` 与 `events.json`，还有若干 `<起始>-<结束>s-<中文动作>.mp4` 片段，一并落在
`/opt/darkpipe/clips/<会话>/` 与回传的 HDFS 目录下。上面这条命令填了 `--gpu_ids 0,1`，但
离线切片依赖单卡连续时间线、与多卡分片互斥——`clip_dir` 默认非空，所以运行时会自动降级为
单卡并打日志说明，不会报错。
...
**没有摄像头、想拿本地文件演示实时模式**时要注意：本地文件（含 HDFS 下载落地的文件）现在一律
判成离线模式——只处理一遍、产出整段视频与切片后退出，填不填 `rtmp_push_url` 都一样，算子本身
不再提供「循环播放本地文件」的演示路径。要演示实时模式，先在算子外用 ffmpeg 把文件循环推成流
（如 `ffmpeg -re -stream_loop -1 -i demo.mp4 -c copy -f flv rtmp://<流媒体服务器>/live/demo`），
再把 `--video_path` 填成对应的拉流地址，或者直接用上面的 rtsp 地址。
...
7. **`video_path` 填文件（本地路径或 HDFS 地址）一律走离线模式**：整段只处理一遍、产出整段
   视频与切片后自行退出，不循环播放；此时 `rtmp_push_url` 被忽略（启动日志会明说）。旧版
   「文件 + 推流地址 = 循环播放演示」的行为已移除，替代做法见上文「没有摄像头、想拿本地文件
   演示实时模式」一节（算子外用 ffmpeg 循环推流）。
