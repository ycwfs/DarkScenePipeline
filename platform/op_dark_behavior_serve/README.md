...
- `gpu_ids` 填的是**容器内**卡号，从 0 起连续编号，与宿主机卡号无关。**默认 `0`，即只用第一张卡**；
  算子仍按 `gpu.count: 2` 申请两张，想用满就把本项改成 `0,1`，不需要重新报备算子。
  单卡跑不出上表的 23.8 fps（840 单卡是 12.9 fps），**默认单卡时 `proc_max_side` 要一并改成 720**。
...
（`other` 默认在 `clip_skip_labels` 里）。**默认 `0.8`**；填 `0` 关闭，按模型原始判定输出。
...
`reco_min_conf` 显示的是**实际生效**的值。不填时它应当是 `0.8`（本算子的默认值）；如果那里是
`0.0` 而你并没有填 `0`，说明平台上注册的还是旧版 `suanzi.json`（旧默认就是 `0.0`）——
**换了 zip 之后需要在平台上重新导入算子**，否则框架按旧 manifest 下发命令行，新默认不会生效。

**怎么定这个值**：默认 `0.8` 是偏保守的取值，宁可漏报不误报。想自己定就先按 `0` 跑一段，
看 `events.jsonl` 里 `confidence` 的实际分布，再挑一个能把
...
| `reco_min_conf` | 行为判定阈值 | Float | `0.8` | 低于该置信度报为「其他」，不显示也不存片段；`0`=关闭 |
| `label_bar` | 叠加标签条 | Bool | `true` | 演示画面与片段底部是否叠加识别结果 |
| `gpu_ids` | GPU卡号 | String | `0` | 逗号分隔，多张时按帧轮询分发；容器内卡号从 0 起编，见下。**默认单卡，须把 `proc_max_side` 改 720** |
