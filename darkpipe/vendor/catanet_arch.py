"""CATANet (CVPR 2025) — verbatim from EquationWalker/CATANet, minus its basicsr couplings.

Two edits only, both mechanical:
  - `@ARCH_REGISTRY.register()` on CATANet dropped (basicsr builds archs by name from a yml;
    we construct the class directly).
  - `trunc_normal_` taken from torch.nn.init instead of basicsr.archs.arch_util, which is
    where basicsr itself re-exports it from.
Everything else, including the layer definitions and the `setting` dict that fixes the
published x2/x3/x4 topology, is unmodified so the released weights load strict=True.
"""
import math
